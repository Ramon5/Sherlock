 ...
 * @author root
 */
public class DropBoxController implements DetectaSistema{
    ...
    public static void validarChaveDropBox(String appKey, String appSec) {
        dropbox = new DropBox();
        auth = new GeradorCredencial();
        String accessToken = dropbox.getAccessToken(appKey, appSec);
        auth.gerarArquivoCrypto(accessToken, System.getProperty("user.home") + "/SherlockTM/Config_Auth/authDropBox.cfg");
        JOptionPane.showMessageDialog(null, "Arquivo de autenticação gerado com sucesso!");
    }

    public static void usarDropBox(boolean selecionado) {

        if (selecionado) {
            dropbox = new DropBox();
            dropbox.carregarAutenticacao();
        } else {
            dropbox.logout();
            dropbox = null;
        }
